...
<template>
    ...
    <a class="member-link" :href="'/profile/' + memberInfo.id">
      <div class="member-avatar-wrp">
        <img class="member-avatar" v-if="memberProfile && memberProfile.main_avatar_url" :src="'/uploads/' + memberInfo.username + '/' + memberProfile.main_avatar_url" alt="member avatar">
      ...
      formatTime(date) {
	      ...
	      var REFERENCE = moment(moment.utc(new Date()).format('DD/MM/yyyy'));
			...
			async fetchMessages() {
				...
				if (data.length) {
					if (lastMessageSid) {
						const targetIndex = data.findIndex(item => item.sid == lastMessageSid);
						if (targetIndex) {
							this.unreadMessages = data.filter((item, index) => index > targetIndex).length;
						}
					} else {
